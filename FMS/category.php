    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
        ...
        if (!empty($targetTable)) {
            ...
            if (move_uploaded_file($fileTmpName, $uploadDir . $fileName)) {
                ...
                $sql = "INSERT INTO $targetTable (NAME, DOWNLOAD, custom_name, description, upload_date) VALUES (?, 0, ?, ?, NOW())";
                $stmt = $conn->prepare($sql);
                if ($stmt) {
                    $stmt->bind_param("sss", $fileName, $customName, $description);
                    $stmt->execute();
...
<body>
    <div class="file-upload-container">
        ...
        <form action="category.php" method="post" enctype="multipart/form-data" class="upload-form">
            ...
            <div class="form-group">
                <label for="target_table">Select Destination:</label><br>
                <select name="target_table" id="target_table">
                    <option value="upload_files">Notice</option>
                    <option value="upload_files1">Policy</option>
                </select>
            </div>
