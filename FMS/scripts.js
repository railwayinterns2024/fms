...
function setupCategoryPage() {
    ...
    links.forEach(link => {
        link.addEventListener('click', (event) => {
            ...
            window.location.href = `subcategory.php?category=${category}`;
        });
    });
}
...
function addCategory() {
    ...
    if (newCategoryName !== "") {
        ...
        newCategoryLink.href = `subcategory.php?category=${newCategoryName.toLowerCase().replace(/\s+/g, '-')}`;
...
function loadSubcategories(category) {
    ...
    subcategoryItems.forEach(sub => {
        ...
        link.href = `documents.php?category=${category}&subcategory=${sub.toLowerCase().replace(/\s+/g, '-')}`;
    ...
    splashVideo.onended = function() {
        ...
        function addCategory() {
            ...
            if (newCategoryName !== "") {
                ...
                newCategoryLink.addEventListener('click', (event) => {
                    event.preventDefault();
                    const category = encodeURIComponent(newCategoryName.toLowerCase().replace(/\s+/g, '-'));
